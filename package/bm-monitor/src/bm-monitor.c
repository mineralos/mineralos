...
#include "bm-stats.h"
#include "bm-socket-server.h"

#define BM_MONITOR_INDENT           ("bm_monitor")
#define BM_MONITOR_GET_DHCP_CMD     ("cat /config/network/25-wired.network | sed -n '/^DHCP/p' | awk -F'=' '{print $2}'")
#define BM_RESET_CONF_CMD           ("rm -rf /config/*; /bin/systemctl reboot")
#define BM_RUN_SELFTEST_CMD         ("/bin/bm-selftest")

#define BM_MONITOR_NET_DHCP         (0)
#define BM_MONITOR_NET_STATIC       (1)
#define BM_MONITOR_NET_ERR          (-1)

#define BM_LED_GREEN                (0x1)
#define BM_LED_RED                  (0x2)
#define BM_LED_ON                   (0)
#define BM_LED_OFF                  (1)
#define BM_QUICK_FLASH_US           (250 * 1000)

#define BM_SEND_IP_ADDR             (INADDR_BROADCAST)

#define BM_INTERFACE_PATH           ("/config/network/25-wired.network")
#define BM_STATIC_IP_ADDR           ("192.168.1.254")
#define BM_NETMASK                  ("24")
#define BM_GATEWAY                  ("192.168.1.1")
#define BM_DNS1                     ("8.8.8.8")
#define BM_DNS2                     ("114.114.114.114")
...
int main(int argc, char *argv[])
{
    ...
    if(!isExist("/sys/class/gpio/gpio943"))
    {
        FILE *pRreco = NULL;
        ...
        if(fwrite("943", strlen("943"), 1, pRreco) != 1)
        {
            printf("File Write Error!\n");
        }
        fclose(pRreco);
        system("echo out > /sys/class/gpio/gpio943/direction");
    }

    if(!isExist("/sys/class/gpio/gpio944"))
    {
        FILE *pRreco = NULL;
        if((pRreco = fopen("/sys/class/gpio/export", "w")) == NULL)
        {
        ...
        if(fwrite("944", strlen("944"), 1, pRreco) != 1)
        {
            printf("File Write Error!\n");
        }
        fclose(pRreco);
        system("echo out > /sys/class/gpio/gpio944/direction");
    }

    if(!isExist("/sys/class/gpio/gpio957"))
    {
        FILE *pRreco = NULL;
        if((pRreco = fopen("/sys/class/gpio/export", "w")) == NULL)
        {
        ...
        if(fwrite("957", strlen("957"), 1, pRreco) != 1)
        {
            printf("File Write Error!\n");
        }
        fclose(pRreco);
...
int get_shell_cmd_rst(char *cmd, char *result, int size)
{
    char buffer[1024] = {0};
    int offset = 0;
    int len;
    ...
    if(NULL == fp)
    {
        printf("failed to open pipe for command %s", cmd);
        return 0;
    }
    ...
    printf("command result(%d): %s", offset, result);
...
static void *thread_button_monitor(void *argv)
{
    ...
    while(1)
    {
        ...
        {   /* 按键按下时执行的操作 */
            ...
            set_red_led(BM_LED_ON);
            set_green_led(BM_LED_ON);
        }
        else
        {   /* 按键弹起时执行的操作 */
            if(s_dhcp_mode)
                revert_led(BM_LED_GREEN);
            else
                set_green_led(BM_LED_ON);
...
static bool get_dhcp_mode(void)
{
    ...
    get_shell_cmd_rst(BM_MONITOR_GET_DHCP_CMD, result, sizeof(result));
...
static void revert_default_conf(void)
{
    const char *cmd = BM_RESET_CONF_CMD;
...
static void change_net_mode(void)
{
    ...
    fp_interface = fopen(BM_INTERFACE_PATH, "w");
...
static int get_button(void)
{
    unsigned int value;
    FILE *pRreco = NULL;
    if((pRreco = fopen("/sys/class/gpio/gpio957/value","r")) == NULL)
    {
...
static void set_green_led(int state)
{
    if(state == BM_LED_ON)
    {
        system("echo 1 > /sys/class/gpio/gpio944/value");
    }
    else 
    {
        system("echo 0 > /sys/class/gpio/gpio944/value");
    }
}
...
static void set_red_led(int state)
{
    if(state == BM_LED_ON)
    {
        system("echo 1 > /sys/class/gpio/gpio943/value");
    }
    else 
    {
        system("echo 0 > /sys/class/gpio/gpio943/value");
    }
}
