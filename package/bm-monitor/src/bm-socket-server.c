...
void *thread_socket_server(void *argv)
{
    ...
    int client = 0;
    char *sendBuf = NULL;
    char recvBuff[BM_RECV_BUF_SIZE] = {0};
    int recvBytes = 0;
    ...
    server_addr.sin_port = htons(BM_MONITOR_PORT);
    ...
    /* 设置服务器上的socket为监听状态 */
    if(listen(serverSocket, BM_LISTEN_NUM) < 0)
    {
        perror("listen");
        return NULL;
    }
    ...
    while(1)
    {
        ...
        bzero(recvBuff, BM_RECV_BUF_SIZE);
        recvBytes = recv(client, recvBuff, BM_RECV_BUF_SIZE, 0);
        if(recvBytes < 0)
        {
            perror("recv");
            continue;
        }
        if(recvBytes >= BM_RECV_BUF_SIZE)
        {
            fprintf(stderr, "BM_RECV_BUF_SIZE is too small.");
            return NULL;
        }
